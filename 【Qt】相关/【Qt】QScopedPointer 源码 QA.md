...
1. QScopedPointer 的大概实现？
- 只是简单地将 RAII 机制实现。
- 注意有几个删除器 QScopedPointerDeleter、QScopedPointerArrayDeleter、QScopedPointerPodDeleter、QScopedPointerObjectDeleteLater，分别可以对 new 出来的对象进行析构、对 new 出来的对象数组进行析构、对 malloc 出来的内存块进行 free、对 new 出来的 QObject 对象进行 deleteLater。

2. sizeof QScopedPointer 是多大？
- 64位平台下是8。因为 QScopedPointer 里面只有一个成员变量。
    ```cpp
    T *d;
    ```
- 但注意 std::unique_ptr 的成员变量是 tuple<pointer, _Dp> _M_t，其大小与删除器的实现是有关的，不一定总是 8 字节（64位平台下）。
